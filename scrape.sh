...
HAS_SUBMISSIONS=$(docker exec pg17-clustr psql -U clipper -d clipper_db -tAc "SELECT EXISTS(SELECT 1 FROM clip_submissions LIMIT 1);")

if [ "$HAS_SUBMISSIONS" = "t" ]; then
    # Let the scraper discover broadcasters from submissions
    docker exec clipper-backend /root/scraper -min-views 50 -max-age-days 7
else
    # Use seed list for fresh DB
    docker exec clipper-backend /root/scraper -min-views 50 -max-age-days 7 -broadcasters "$BROADCASTERS"
fi
