...
func (s *SubmissionService) ApproveSubmission(ctx context.Context, submissionID, reviewerID uuid.UUID) error {
	preparedClips, err := s.prepareApprovalClips(ctx, []uuid.UUID{submissionID})
	if err != nil {
		return fmt.Errorf("failed to prepare submission approval: %w", err)
	}
	submissions, err := s.submissionRepo.ModerateAtomically(ctx, []uuid.UUID{submissionID}, reviewerID, true, nil, preparedClips)
	if err != nil {
...
func (s *SubmissionService) RejectSubmission(ctx context.Context, submissionID, reviewerID uuid.UUID, reason string) error {
	submissions, err := s.submissionRepo.ModerateAtomically(ctx, []uuid.UUID{submissionID}, reviewerID, false, &reason, nil)
	if err != nil {
...
func (s *SubmissionService) BulkApproveSubmissions(ctx context.Context, submissionIDs []uuid.UUID, reviewerID uuid.UUID) error {
	preparedClips, err := s.prepareApprovalClips(ctx, submissionIDs)
	if err != nil {
		return fmt.Errorf("failed to prepare bulk submission approval: %w", err)
	}
	submissions, err := s.submissionRepo.ModerateAtomically(ctx, submissionIDs, reviewerID, true, nil, preparedClips)
	if err != nil {
...
func (s *SubmissionService) BulkRejectSubmissions(ctx context.Context, submissionIDs []uuid.UUID, reviewerID uuid.UUID, reason string) error {
	_, err := s.submissionRepo.ModerateAtomically(ctx, submissionIDs, reviewerID, false, &reason, nil)
	if err != nil {
...
func (s *SubmissionService) prepareApprovalClips(ctx context.Context, submissionIDs []uuid.UUID) (map[uuid.UUID]*models.Clip, error) {
	...
	prepared := make(map[uuid.UUID]*models.Clip, len(submissions))
	for _, submission := range submissions {
		if submission.Status != "pending" {
			return nil, repository.ErrSubmissionNotPending
		}
