...
describe('CommunityRulesPage', () => {
    ...
    it('renders all 7 main rules', () => {
        ...
        expect(
            screen.getByText(/1\. Be Respectful and Kind/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/2\. Share Authentic Content/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/3\. No Spam or Self-Promotion Abuse/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/4\. Keep Content Safe and Appropriate/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/5\. Respect Privacy and Personal Information/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/6\. Follow Platform and Legal Guidelines/i),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/7\. Report Issues and Help Moderate/i),
        ).toBeInTheDocument();
    });
    ...
    it('has anchor IDs for navigation', () => {
        ...
        expect(
            container.querySelector('#authentic-content'),
        ).toBeInTheDocument();
        ...
        expect(
            container.querySelector('#platform-guidelines'),
        ).toBeInTheDocument();
    ...
    it('includes enforcement and consequences section', () => {
        ...
        expect(
            screen.getByText('Enforcement and Consequences'),
        ).toBeInTheDocument();
    ...
    it('mentions key values and expectations', () => {
        ...
        expect(
            screen.getByText(
                /No harassment, hate speech, or discriminatory language/i,
            ),
        ).toBeInTheDocument();
        expect(
            screen.getByText(
                /Only submit clips from legitimate Twitch streams/i,
            ),
        ).toBeInTheDocument();
        expect(
            screen.getByText(/No NSFW \(Not Safe For Work\) content/i),
        ).toBeInTheDocument();
    });
    ...
    it('provides contact information', () => {
        ...
        expect(
            screen.getByText(/Questions or Concerns\?/i),
        ).toBeInTheDocument();
        expect(
            screen.getByRole('link', { name: /support@clpr.tv/i }),
        ).toHaveAttribute('href', 'mailto:support@clpr.tv');
    });
