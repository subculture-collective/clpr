...
- **State:** IN PROGRESS — all 467 registered release routes have matching operations and schema-bearing responses (483 total documented operations, including debug/spec-only operations); CI enforces parity, response structure, a route-to-handler manifest, and a non-increasing transitional-contract budget; 17 router-derived operations remain, all in the administrative API family
...
- **State:** BLOCKED — fail-closed evidence workflow reports 17 transitional administrative API contracts plus missing Stripe test-mode, staging load/soak, production restore, and canary/rollback artifacts
...
### Refined administrative lifecycle story

**US-ADM1 — Safe, accountable user administration.** As an administrator, I can
manage non-administrator accounts without risking privilege confusion, partial
changes, or unaudited actions. A moderator cannot receive global user-management
authority; malformed identity or input fails closed; self-targeting and ordinary
changes to administrator accounts are denied; every successful ban, role, karma,
badge, comment-suspension, or review-policy mutation commits with its audit record;
and duplicate or missing badge operations return stable conflict/not-found results.
The route-specific API contracts and an authorization matrix must prove these
behaviors before launch.
...
| US-ADM1 — safe user administration | R2.4, R3.6, R3.7 |
...
| 2026-07-12 | R2.4/R3.7 filter presets | Partial | All five preset operations now have route-specific contracts and focused malformed-input/identity tests; route-user identity cannot panic and is enforced consistently, cross-user preset IDs conceal existence, empty updates fail closed, nested arrays/strings/language/sort/date ranges are bounded and validated, the ten-preset cap is serialized with a per-user transaction lock, raw errors are replaced by stable 403/404/409 responses, and public responses expose typed filters rather than double-encoded storage JSON; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 190 to 185 |
| 2026-07-12 | R2.4/R3.7 followed games and verification submission | Partial | The followed-games route now consumes its registered `{id}` parameter and strictly bounds pagination; both user verification operations plus followed games have route-specific contracts and focused route/input/identity tests; verification requires an HTTPS single-channel Twitch URL, bounds audience/social fields, safely validates identity, and atomically serializes verified/pending/cooldown/five-attempt/duplicate-channel checks plus insertion under user and normalized-channel locks with stable 409/429 outcomes; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 185 to 182 |
| 2026-07-12 | R2.4/R3.7 Stripe webhook ingress | Partial | The public Stripe webhook operation now has a route-specific contract and focused ingress tests; payloads over 1 MiB and signatures over 8 KiB fail before service work, missing webhook configuration fails closed, invalid signatures return a redacted 400, internal claim/processing failures return 500 so Stripe retries, successful/duplicate receipts return a minimal response, and production processing retains atomic cross-instance event claims; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 182 to 181 |
| 2026-07-12 | R2.4/R3.7 chat lifecycle | Partial | `6f06bbb2`; all 21 chat operations now have route-specific contracts; private channel lists, details, members, history, and WebSocket entry require membership while public channels remain visible; moderation is authorized by channel owner/admin/moderator membership instead of unrelated application roles, restriction/message mutations and audit records commit atomically, malformed identity/filter/pagination input fails closed, and concurrent invitations serialize enforcement of `max_participants`; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 181 to 160 |
| 2026-07-12 | R2.4/R3.7 community lifecycle | Partial | `7cbf4a26`; all 21 community operations now have route-specific contracts and focused malformed-input/identity tests; public search excludes private records, legacy private community details/members/feeds/discussions require existing membership, nested discussion IDs must belong to the route's parent community, hidden/removed clips are excluded, exact clip authorization replaces a 100-item scan, creation plus owner membership and bans plus membership removal commit atomically, missing removals no longer report success, pagination/roles/sorts/updates fail closed, and raw errors are redacted; the absent private invitation and product UI lifecycle is contained and documented as partial, so new private communities are rejected; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 160 to 139 |
| 2026-07-12 | R2.4/R3.7 forum lifecycle | Partial | `26d7f81b`; all fourteen forum operations now have route-specific contracts and focused malformed-input/identity tests; invalid pagination/sorts/timeframes/limits and raw binder details fail closed, tags are bounded and unique, reply hierarchy ordering is deterministic, hidden/spam/deleted replies and replies under deleted threads are excluded from trees/search/votes/helpful results/analytics, nonexistent reputation users return 404, broken game-column analytics queries are repaired, reply creation serializes against thread locking, votes plus reputation updates commit atomically under a reply lock, and content flags verify and lock visible non-self targets through insertion; the feature inventory now records categories and subscriptions as incomplete rather than calling the forum complete; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 139 to 125 |
| 2026-07-12 | R2.4/R3.7 public product discovery | Partial | `b73131cb`; all 21 ads/broadcasters/categories/clip-processing/games operations now have route-specific contracts and focused validation/redaction tests; malformed identity and pagination/filter inputs fail closed, public ad responses use a creative-only DTO, impression state is monotonic and first-view charging commits exactly once with the impression, optional ad/category identity is actually wired through routing, category clips use one exact visible-membership query without unrelated fallback or fan-out truncation, public broadcaster/category/game counts exclude hidden clips, game and broadcaster IDs are bounded and missing unfollows return 404, media redirects are non-cacheable, and processing status no longer leaks internal job payloads or converts dependency failure into `not_queued`; full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 125 to 104 |
| 2026-07-12 | R2.3/R3.7 operational and server-rendered routes | Partial | `21b20c5e`; all fifteen health/internal/profiler/best-of operations now have explicit contracts; the eleven Go profiler routes are absent from release-mode routers, remain admin-authenticated in explicit debug mode, and are documented as development-only; database and Prometheus operations declare and retain dedicated operational-token authentication; basic health remains minimal and public; best-of wildcard routing has an explicit HTML period/archive contract; the release route manifest decreases from 480 to 469 registered routes, full Go test/vet and the 84-warning OpenAPI baseline pass, and transitional operations are now exclusively the 89-operation admin family |
| 2026-07-12 | R2.4/R3.6/R3.7 administrative user lifecycle | Partial | `2f6870f4`, `7214a8bc`; all eleven user-management operations now have explicit contracts; global user management is removed from moderator permissions; malformed filters, pagination, identity, reasons, roles, karma, suspension, and badge inputs fail closed; self-targeting and ordinary administrator-account mutations are denied; user, badge, suspension, review-policy, and audit changes share transactions; duplicate/missing badge outcomes are explicit; focused regressions plus full Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 73 to 62 |
...
| 2026-07-12 | R2.4/R3.7 advertising administration | Partial | `017c8f27`; all twelve advertising-administration operations now have explicit administrator-only contracts; moderator access is removed, mutations document CSRF, campaign names/creative URLs/dimensions/priorities/weights/budgets/targeting payloads/dates are bounded, empty updates fail closed, missing deletes map to 404, client and dependency failures are separated without leaking internals, report ranges are ordered and capped at 365 days, and targeting-rule dependency failures can no longer admit ads. Focused Go test/vet and the 84-warning OpenAPI baseline pass; transitional operations reduced from 43 to 31 |
| 2026-07-12 | R2.4/R3.7 administrative analytics | Partial | `e8693eea`; all six supported analytics operations now have explicit administrator-only contracts; malformed and unsupported trend metrics/durations fail before repository work, all reads document MFA/403 behavior, and moderators can no longer access platform-wide analytics. The advertised export endpoint returned only a placeholder rather than a file or dataset, so it is removed from release routing/spec until implemented; release routes decrease from 468 to 467 and transitional operations from 31 to 24. Focused Go tests and the 84-warning OpenAPI baseline pass |
